...
10. **Splits into sections** (each open vial event)
	...
	**Get stats for each event**
	- For each vial:
		- Get event data (`d_olfa_flow(vial).events.OV`), flow data (`d_olfa_flow(vial).flow`), and initialize empty structures
		- For each event:
	...
	**Copy legitimate events into OV_keep**  
	...
	- For each vial:
		- Get event data (`d_olfa_flow(vial).events.OV`)
		- For each event: copy all the information into `d_olfa_flow(vial).events.OV_keep`
